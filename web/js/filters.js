...
const SESSN = { Q: "Qualifying", S: "Sprint", R: "Race" };
const evLabel = (c) => `${SESSN[c[0]] || c[0]} ${EVLABEL[c.slice(2)] || c.slice(2)}`;
function fprops(scope) {
  if (scope === "calc")
    return [
      ["score", "Team xPts"],
      ["cost", "Cost ($m)"],
...
const filters = (scope) => ((state.filters || {})[scope] || []).filter((f) => f.min != null || f.max != null);
function filterUI(scope) {
...
const teamText = (label, cons, drs, boost, x3, cost, pts) =>
