...
if (!key) { console.error("LEAGUE_KEY not set"); process.exit(2); }
let input = "";
process.stdin.on("data", (c) => (input += c));
process.stdin.on("end", () => {
  const iter = 250000, salt = crypto.randomBytes(16), iv = crypto.randomBytes(12);
  ...
  process.stdout.write(JSON.stringify({ v: 1, iter, salt: salt.toString("base64"), iv: iv.toString("base64"), ct: ct.toString("base64") }));
});
